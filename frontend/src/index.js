...
class PokemonList extends Component{
  ...
  componentWillMount(){
    ...
    fetch('http://pokeapi.co/api/v2/pokemon?limit=100').then(res=>res.json())
    .then(response=>{
      this.setState({
  ...
  render(){
    ...
    if(fetched){
      content = <div className="pokemon--species--list">
        {
          species.map((pokemon,index)=><Pokemon key={pokemon.name
        } id={index+1} pokemon={pokemon}/>)} </div>;
...
class PokeApp extends Component{
  render(){
    return <div className="pokeapp">
      <h1> The First PokeDex maked in FARM Stack </h1>
      <PokemonList/>
    </div>;
  }
}
